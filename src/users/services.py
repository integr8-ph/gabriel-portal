...
async def get_all_users_in_db() -> Optional[CompleteUserOut]:
    return await User.find().to_list()


async def update_user_in_db(user: User, details: UserUpdate) -> CompleteUserUpdateOut:
...
async def delete_user_in_db(user: User) -> CompleteUserDeleteOut:
    await user.delete()
